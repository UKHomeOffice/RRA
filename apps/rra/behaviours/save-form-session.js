  ...
  saveValues(req, res, next) {
    super.saveValues(req, res, err => {
      ...
      request.post({
        ...
        if (req.body['save-and-exit']) {
          return res.redirect('/rra/save-and-exit');
        }
        ...
        const loopedFieldMatchesForkCondition = req.form.values[loopedForkField] === loopedForkCondition;

        if (req.sessionModel.get('redirect-to-reports') && !isContinueOnEdit && !loopedFieldMatchesForkCondition) {
          return res.redirect(`/rra/continue-report?id=${req.sessionModel.get('id')}`);
        }
